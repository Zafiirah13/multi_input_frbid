...
def model_prediction(fit_model, odir, model_name, X_test_dm, X_test_freq, y_test, classes=["RFI" , "FRB"], cm_norm=False,load_model=False,show=True):
    '''
    ...
    ypred          = np.argmax(fit_model.predict([X_test_dm, X_test_freq]),axis=1)
    probability    = fit_model.predict([X_test_dm, X_test_freq])
