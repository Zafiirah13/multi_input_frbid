...
def feature_maps(model, x_train, y_train, img_index, ofname,input_image, show=True):
    '''
    ...
    if (input_image == 'DM'):
        redefine_model = Model(inputs=model.inputs, outputs=model.layers[2].output)
        feature_maps   = redefine_model.predict(x_train)
        print(feature_maps.shape)
    elif (input_image == 'FREQ'):
        redefine_model = Model(inputs=model.inputs, outputs=model.layers[3].output)
        feature_maps   = redefine_model.predict(x_train)
        print(feature_maps.shape)
